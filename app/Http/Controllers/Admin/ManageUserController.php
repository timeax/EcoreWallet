...
class ManageUserController extends Controller
{
    ...
    public function modifyBalance(Request $request)
    {
        ...
        $wallet = Wallet::where('id', $request->wallet_id)->where('user_id', $request->user_id)->with('curr')->firstOrFail();
        ...
        if ($request->type == 1) {
            ...
            $user->notify(Notifications::credit([
        ...
        if ($request->type == 2) {
            ...
            $user->notify(Notifications::debit([
                'amount' => numFormat($request->amount, 8),
                'curr'  => $wallet->curr->code,
                'trnx'  => $trnx->trnx,
                'after_balance' => numFormat($wallet->balance, 8),
