...
const carousel: CarouselPassThroughOptions = {
    ...
    itemscontent: 'overflow-hidden w-fit',
    //@ts-ignore
    itemscontainer: ({ props }) => ({
        className: classNames('flex ', {
            'flex-row gap-x-3 h-full': props.orientation !== 'vertical',
    ...
    item: ({ props }) => {
        return {
            className: classNames('flex shrink-0 grow', {
                'mr-4': props.orientation !== 'vertical',
                'w-full': props.orientation == 'vertical'
            })
        }
    },
    indicators: {
        className: classNames('flex flex-row justify-start flex-wrap')
    },
    //@ts-ignore
    indicator: 'mr-2 mb-2 rounded-[1px]',
    //@ts-ignore
    indicatorbutton: ({ context }) => ({
        className: classNames('w-8 h-1 transition duration-200 rounded-0', 'focus:outline-none focus:outline-offset-0 focus:shadow-[0_0_0_0.2rem_rgba(191,219,254,1)] dark:focus:shadow-[0_0_0_0.2rem_rgba(147,197,253,0.5)]', {
